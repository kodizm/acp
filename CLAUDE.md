...
- **`subagent_complete` does not fire for a background agent.** When the CLI launches the Task tool asynchronously, the `tool_result` is an "Async agent launched successfully" notice carrying `agentId:` and no `<usage>` block, so only `subagent_spawn` reaches the orchestrator. Measured on CLI 2.1.280 with both SDK 0.3.237 and 0.3.280, so it is not an upgrade regression. Production recorded no Task tool call in the 30 days before 2026-09-23, which is why nothing noticed. `test/integration/_local-bin-task-tool.smoke.ts` reproduces it.
- **`usage.costUsd` is cumulative on the claude backend from CLI 2.1.277.** Each turn is a fresh `query()` with `resume`, and 2.1.277 made a resumed session's `total_cost_usd` continue from the earlier turns instead of starting at zero, so turn N reports turns 1..N. Token counts are still per turn. The orchestrator is unaffected today only because `StreamEventPersister::applyCustomPricing` recomputes cost from tokens for every session with a priced model alias (all of them on production). A consumer that sums `costUsd` directly would over-count.
