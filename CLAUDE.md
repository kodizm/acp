...
- **`session/load` carries the same options as `session/new`** (`SessionOptionFields` in `src/wire/schemas.ts`). A load is a fresh `query()` with `resume`, and CLI 2.1.280 restores only the model and the system prompt from the transcript; the tool policy, permission mode, extra directories and env are per-query. Before 0.6.8 a load carried none of them, so a resumed session ran on `bypassPermissions` with every tool its role denied. The claude driver forwards `systemPrompt` and `model` on a load only when the wire names them, so a bare load does not overwrite the transcript's.
- `features` (new / load / fork) switches built-in families off; an absent key means on. The claude driver maps them in `src/backends/claude/features.ts`: tool families go to `disallowedTools` by bare name, because only a bare name removes the tool from the model's context (a scoped pattern still advertises it); behaviour families go to `CLAUDE_CODE_DISABLE_*` env. `simple` replaces the base built-in set with WebFetch + WebSearch and turns filesystem settings off, since the image's user settings deny both web tools. The opencode driver ignores `features` today. The schemas are plain `z.object`, so a bridge older than the field strips it silently: ship the bridge before the orchestrator relies on it.
...
- Claude bridge detects subagent spawn / complete inline from the Task tool: a `tool_use.name` of `'Agent'` (current CLIs) or `'Task'` (older) triggers `subagent_spawn`; the matching `tool_result` is scanned for `agentId:` plus a `<usage>...</usage>` marker to fire `subagent_complete`. Token-split fields default to 0; `subagent_type` defaults to `'general-purpose'`. Lives in `src/backends/claude/event-mapper.ts`.
- **`subagent_complete` does not fire for a background agent.** Claude Code runs Task agents in the background by default (2.1.198); the `tool_result` is then an "Async agent launched successfully" notice carrying `agentId:` and no `<usage>` block, so only `subagent_spawn` reaches the orchestrator. The model can still run one call in the foreground (the Agent tool's `run_in_background: false`), and `CLAUDE_CODE_DISABLE_BACKGROUND_TASKS=1` is the only operator-side switch. `Explore` and `Plan` agents never carry the markers, foreground or not, so they never complete on the wire. A foreground result was silent too until 0.6.7, for a different reason: the CLI renamed the usage field from `total_tokens` to `subagent_tokens`, and the parser read only the old name. Production recorded no Task tool call in the 30 days before 2026-09-23, which is why nothing noticed. `test/integration/_local-bin-task-tool.smoke.ts` reproduces both.
- **`usage.costUsd` is cumulative on the claude backend from CLI 2.1.277.** Each turn is a fresh `query()` with `resume`, and 2.1.277 made a resumed session's `total_cost_usd` continue from the earlier turns instead of starting at zero, so turn N reports turns 1..N. Token counts are still per turn. The orchestrator prices from tokens instead: `StreamEventPersister::applyCustomPricing` for the session total and `turnCostUsd` for the per-turn figure the conversation view shows, which read `costUsd` straight off the event until 2026-09-23. Any new consumer has to do the same; summing `costUsd` over-counts.
...
| `references/typescript-sdk/` | ACP protocol TypeScript SDK source. Authoritative for protocol types and method names. The wire in `src/server/` and `src/wire/` is hand-rolled, so `@agentclientprotocol/sdk` is deliberately not a dependency: it sat in `package.json` for months without a single import. |
